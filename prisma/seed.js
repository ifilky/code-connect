...
async function main() {
    ...
    const ana = await prisma.user.upsert({ // upsert: ou ele vai atualizar, ou vai inserir (caso não esteja vazio)
        where: { username: author.username }, // para que a função execute corretamente, essa consulta deve ser em um campo único (username)
        update: {},
        create: author,
    })
