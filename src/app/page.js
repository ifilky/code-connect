...
async function getAllPosts(page, searchTerm) {
  try {
    ...
    if (searchTerm) {
      where.title = {
        contains: searchTerm,
        mode: 'insensitive', // permite buscar com letras maiúsculas e minúsculas
      }
    }
...
export default async function Home({ searchParams }) {
  const currentPage = parseInt(searchParams?.page || 1) // busca a page atual e no caso de não haver nenhuma, retorna a page 1
  const searchTerm = searchParams?.q // pega o valor do q (name do input do searchForm)
